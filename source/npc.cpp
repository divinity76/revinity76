...
int NpcScript::registerFunctions()
{
	...
	lua_register(luaState, "getHasMoney", NpcScript::luaMoney);
#endif
	...
	lua_register(luaState, "addMaxHealth", NpcScript::luaAddMaxHealth);
    lua_register(luaState, "addMaxMana", NpcScript::luaAddMaxMana);
#endif //YUR_NPC_EXT
...
int NpcScript::luaAddMaxHealth(lua_State *L)
{
    int cid = (int)lua_tonumber(L, -2);
    unsigned int amount = (unsigned int)lua_tonumber(L, -1);
    lua_pop(L, 2);
    ...
    if(player){
        if(amount > 0){
            ...
            lua_pushnumber(L, -1);
            std::cout << "addMaxHealth: amount is too low" << std::endl;
            return 1; 
        }
    }
    else{
        lua_pushnumber(L, -1);
        std::cout << "addMaxHealth: player not found" << std::endl;
        return 1;
    }
    lua_pushnumber(L, 0);
    return 1;
...
int NpcScript::luaAddMaxMana(lua_State *L)
{
    int cid = (int)lua_tonumber(L, -2);
    unsigned int amount = (unsigned int)lua_tonumber(L, -1);
    lua_pop(L, 2);
    ...
    if(player){
        if(amount > 0){
            ...
            lua_pushnumber(L, -1);
            std::cout << "addMaxMana: amount is too low" << std::endl;
            return 1; 
        }
    }
    else{
        lua_pushnumber(L, -1);
        std::cout << "addMaxMana: player not found" << std::endl;
        return 1;
    }
    lua_pushnumber(L, 0);
    return 1;
...
int NpcScript::luaMoney(lua_State *L)
{
	int cost = (int)lua_tonumber(L, -1);
	int cid = (int)lua_tonumber(L, -2);
	lua_pop(L,2);
	...
	if (player)
	{
		if (player->getCoins(cost)) { return 1 ; }
        else { return 0 ; }
    }
	else return 0;
}
